...
func _process(delta: float) -> void:
	...
	for peer_id in _stun_immunity.keys():
	...
	for peer_id in _post_stun_dr.keys():
...
func apply(player_node: Node, effect_name: String, params: Dictionary) -> void:
	...
	if instances.size() > 0:
		...
		if effect_name == "slow":
			instances[0]["magnitude"] = params.get("magnitude", instances[0]["magnitude"])
		if effect_name == "stun" and params.has("post_stun_dr"):
		...
		if effect_name == "stun":
			instance["timer_original"] = duration   # ← AÑADIR ESTA LÍNEA
...
func register(peer_id: int) -> void:
	_ensure_registered(peer_id)
...
func _on_stun_expired(peer_id: int, instance: Dictionary) -> void:
	...
	if player_node.character_data and player_node.character_data.team == "killer":
		# Killer: inmunidad post-stun automática
		...
		# Survivor: DR post-stun solo si la habilidad lo pidió
