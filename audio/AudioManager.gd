...
const CHASE_RADIUS_BASE := 200.0
const CHASE_RADIUS_EXPANDED := 400.0
...
func _update_proximities_from_local(local_player: Node, delta: float) -> void:
	...
	var is_killer: bool = local_player.is_in_group("killer")
	var distance: float = local_player.global_position.distance_to(killer_node.global_position)

	if is_killer:
		# ── Killer: solo chase al acercarse a un survivor ──
		for s in survivors:
			if not is_instance_valid(s): continue
			var d: float = local_player.global_position.distance_to(s.global_position)
	...
	# ── Survivor: chase → mapa+alerta, chase silencia mapa ──
	var chase_r = CHASE_RADIUS_EXPANDED if _is_chasing else CHASE_RADIUS_BASE
...
func _rpc_activate_lms_audio(survivor_peer_id: int) -> void:
	...
	if is_instance_valid(survivor_node) and "character_data" in survivor_node:
		...
		if char_data and char_data.lms_music:
			...
			_set_lms_stream(char_data.lms_music)
